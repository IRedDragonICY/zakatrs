...
fn test_restricted_funds_integration() {
    ...
    config.strategy.pension_zakat_on_vested = true;
...
fn test_advanced_receivables_integration() {
    ...
    // If Gold Nisab ~ 85g * $60 = $5100 (approx, depends on default)
    // Default config gold is usually set low or 0 in tests requiring set. 
    // Wait, BusinessZakat tests use 100/g. 85 * 100 = 8500.
    // Here we use default. Let's force price.
...
fn test_distribution_integration() {
    let needs = BasicNeeds {
        food: dec!(500),
        shelter: dec!(1000), // Total 1500 + etc
        ..Default::default() 
    };
